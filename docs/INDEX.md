...
| 部署到服务器或云平台 | [部署指南](DEPLOY.md) | [云端 WebUI 部署](deploy-webui-cloud.md)、[Zeabur 部署](docker/zeabur-deployment.md)、[完整部署与使用手册](full-deployment-zh.md) |
...
| [A 股量化策略研究备忘](strategy-research/a-share-quant-gaps.md) | 12 个公开有效但项目未收录的 A 股策略盘点（多因子/动量/反转/PEAD/海龟/网格/北向等），含学术证据、推荐优先级、工具设计、验证方法与未验证项声明 |
...
| [完整部署与使用手册](full-deployment-zh.md) | 端到端实战手册：本地 + Docker 云部署 + GitHub Actions、钉钉 Bot、LLM / 数据源 / 通知 / Agent、安全实践、排障 |
